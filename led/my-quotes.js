...
async function showQuoteDetails(quote) {
  ...
  modalContent.innerHTML = `
    <div id="quote-details-content-wrapper" style="background-color: #f8f9fa; border-radius: 8px; padding: 25px; max-width: 900px; margin: 20px auto; box-shadow: 0 4px 15px rgba(0,0,0,0.1); position: relative;"> <!-- Wrapper for styling -->
        ...
        <div class="section approval-section" style="text-align: center; margin-top: 30px; display: flex; justify-content: center; gap: 15px;">
           <button id="share-quote-btn" style="padding: 12px 25px; font-size: 1rem; color: #fff; background-color: #6c757d; border: none; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 8px;">
           ...
           <button id="approve-quote-btn" class="form-submit" data-proposal-id="${proposalId}" style="padding: 12px 25px; font-size: 1rem; color: #fff; background-color: #28a745; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.2s ease; margin: 0;">Aprovar Orçamento</button>
        </div>
...
async function checkCalendarAvailability(quoteData) {
  try {
    const endpointUrl = '/api/check-availability';

    const payload = {
      quoteId: quoteData.id,
      startDate: quoteData.shooting_dates_start || quoteData.shooting_date,
      endDate: quoteData.shooting_dates_end || quoteData.shooting_dates_start || quoteData.shooting_date
    };

    console.log('Checking availability via local API:', payload);
